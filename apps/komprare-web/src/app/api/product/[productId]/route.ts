...
export async function GET(
  ...
  try {
    ...
    // Process results
    const belgium =
      beResult.status === 'fulfilled' && !isScraperError(beResult.value)
        ? beResult.value
        : null;

    const netherlands =
      nlResult.status === 'fulfilled' && !isScraperError(nlResult.value)
        ? nlResult.value
        : null;

    const france =
      frResult.status === 'fulfilled' && !isScraperError(frResult.value)
        ? frResult.value
        : null;

    const germany =
      deResult.status === 'fulfilled' && !isScraperError(deResult.value)
        ? deResult.value
        : null;
