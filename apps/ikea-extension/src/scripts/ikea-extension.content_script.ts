  ...
  loading.innerHTML = `
    <div>
      <div style="background: #0058A3; padding: 16px; border-bottom: 2px solid #000;">
        <img src="${chrome.runtime.getURL('assets/logo.png')}" alt="KOMPRÅRE" style="height: 50px; display: block; margin: 0 auto;" />
      </div>
...
function createPriceTable(prices: Record<string, PriceData>): HTMLElement {
  ...
  countries.forEach((country, index) => {
    ...
    if (belgianPrice && country !== 'BE') {
      ...
      if (diff < 0) {
        priceDiff = `<span style="color: #16a34a; font-size: 12px; font-weight: 600;">-€${Math.abs(diff).toFixed(2)}</span>`;
      } else if (diff > 0) {
        priceDiff = `<span style="color: #dc2626; font-size: 12px; font-weight: 600;">+€${diff.toFixed(2)}</span>`;
      }
    }
    ...
    if (priceData.storeAvailability) {
      ...
      if (quantity > 0) {
        ...
        else if (stockLevel === 'MEDIUM_IN_STOCK') stockColor = '#eab308'; // yellow
      ...
      if (quantity > 0) {
        ...
        const restockDate = new Date(avail.cashCarry.restockDate).toLocaleDateString('nl-NL', {
          day: 'numeric',
          month: 'short'
        });
        availabilityHTML = `
    ...
    row.innerHTML = `
      <div style="display: flex; align-items: center; gap: 12px;">
        ...
        <div>
          ...
          ${isCheapest ? '<div style="display: inline-block; background: #E91E63; color: white; padding: 2px 6px; font-size: 10px; font-weight: 700; letter-spacing: 0.5px;">BESTE PRIJS</div>' : ''}
 ...
 */
async function fetchPrices(productId: string): Promise<Record<string, PriceData>> {
...
async function updateWidget(productId: string) {
  ...
  try {
    ...
    widget.innerHTML = '';
    widget.appendChild(createErrorState(error instanceof Error ? error.message : 'Onbekende fout'));
  }
}
