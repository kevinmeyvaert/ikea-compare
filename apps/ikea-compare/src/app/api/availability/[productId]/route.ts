...
async function fetchStoreAvailability(
  ...
  try {
    ...
    console.log(`[Availability API] Fetching from IKEA API for store ${storeId} (${storeName}), product ${productId}`);
    ...
    console.log(`[Availability API] Got ${response.data.availabilities?.length || 0} store results`);
    ...
    let quantity = 0;
    let available = false;
    let stockLevel: 'HIGH_IN_STOCK' | 'MEDIUM_IN_STOCK' | 'LOW_IN_STOCK' | 'OUT_OF_STOCK' | 'UNKNOWN' = 'UNKNOWN';
    ...
    if (data?.availabilities && Array.isArray(data.availabilities)) {
      ...
      const storeAvailability = data.availabilities.find((avail: any) =>
        avail.classUnitKey?.classUnitCode === storeId
      );

      if (storeAvailability) {
        console.log(`[Availability API] Found store ${storeId}:`, JSON.stringify(storeAvailability, null, 2));
    ...
    console.error(`[Availability API] Error:`, error.response?.data || error.message);
