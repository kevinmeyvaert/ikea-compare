...
export default function StatsCard() {
  ...
  const fetchStatistics = async () => {
    ...
    try {
      ...
      const countryStatsSnapshot = await getDocs(collection(db, 'country-stats'));
      const countryStats: CountryStat[] = countryStatsSnapshot.docs.map(doc => {
        const data = doc.data();
        const avgPriceDifference = data.comparisonCount > 0
          ? data.totalPriceDifference / data.comparisonCount
          : 0;
        const cheapestPercentage = data.comparisonCount > 0
          ? (data.timesWasCheapest / data.comparisonCount) * 100
          : 0;
        const mostExpensivePercentage = data.comparisonCount > 0
      ...
      const topProducts: ProductStat[] = productStatsSnapshot.docs.map(doc => {
        const data = doc.data();
        return {
      ...
      const percentageInAllCountries = totalProducts > 0
        ? (availableInAllCountries / totalProducts) * 100
        : 0;
      ...
      <div className="bg-black px-6 lg:px-8 py-3 mb-6 -mx-6 lg:-mx-8 -mt-6 lg:-mt-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex-1 min-w-[120px]">
              <div className="text-[10px] text-gray-400 mb-0.5">
                Totale besparing <span className="text-gray-500">vs België</span>
              </div>
              <div className="text-lg font-bold text-white">
            ...
            <div className="flex-1 min-w-[120px]">
              <div className="text-[10px] text-gray-400 mb-0.5">
                Gem. besparing per product <span className="text-gray-500">vs België</span>
              </div>
              <div className="text-lg font-bold text-white">
