...
const storeSelectorsEl = document.getElementById('store-selectors') as HTMLDivElement;
...
function populateStoreDropdowns(): void {
  const countries: Array<{ code: 'BE' | 'NL' | 'FR' | 'DE'; element: HTMLSelectElement }> = [
...
async function savePreferences(): Promise<void> {
  try {
    ...
    tabs.forEach(tab => {
      if (tab.id) {
        chrome.tabs.sendMessage(tab.id, { type: 'STORE_PREFERENCES_UPDATED' }).catch(() => {
          // Tab might not have content script loaded yet, ignore
        });
      }
    });

    console.log('[Popup] Preferences saved successfully, notified', tabs.length, 'tabs');
