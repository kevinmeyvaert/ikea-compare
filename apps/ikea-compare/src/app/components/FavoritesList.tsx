...
export default function FavoritesList({ onProductClick, onFavoriteRemoved }: FavoritesListProps) {
  const [favorites, setFavorites] = useState<FavoriteProduct[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const isAuthenticated = useAuth();

  useEffect(() => {
    console.log('[FavoritesList] Auth state changed, isAuthenticated:', isAuthenticated);
    if (isAuthenticated) {
      loadFavorites();
    }
  }, [isAuthenticated]);
  ...
  const handleRemoveFavorite = async (productId: string, event: React.MouseEvent) => {
