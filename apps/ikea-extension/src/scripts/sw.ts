...
async function initializeAuth() {
  ...
  authPromise = (async () => {
    try {
      ...
      console.log('[Firebase SW] Anonymous auth initialized:', userCredential.user.uid);

      // Verify auth.currentUser is set
      if (auth.currentUser) {
        console.log('[Firebase SW] auth.currentUser confirmed:', auth.currentUser.uid);
...
/**
 * Build product URL for a country
 */
function buildProductUrl(country: keyof typeof COUNTRY_CONFIGS, productId: string): string {
  const config = COUNTRY_CONFIGS[country];
  return `${config.baseUrl}/p/${productId}/`;
}

/**
 * Extract product name and image from HTML (used as fallback)
 */
function extractProductMetadata(html: string): { name: string; imageUrl: string } {
  let name = '';
  let imageUrl = '';
  ...
  // Fallback: Try to extract from meta tags or page title
  if (!name) {
    const titleMatch = html.match(/<title[^>]*>([^<]+)<\/title>/i);
    if (titleMatch) {
      name = titleMatch[1].replace(/\s*-\s*IKEA.*$/i, '').trim();
    }
  }
...
/**
 * Extract product info (price, name, image) from IKEA page HTML
 */
function extractPriceFromHTML(html: string, country: keyof typeof COUNTRY_CONFIGS): { price: number; currency: string; available: boolean; name?: string; imageUrl?: string } | null {
  try {
    // Extract product metadata (name, image) first - this will be used as fallback
    const metadata = extractProductMetadata(html);

    // Strategy 1: Look for JSON-LD structured data (multiple script tags)
    const jsonLdRegex = /<script type="application\/ld\+json">([\s\S]*?)<\/script>/g;
    let match;
    while ((match = jsonLdRegex.exec(html)) !== null) {
      try {
        ...
        if (jsonData['@type'] === 'Product' && jsonData.offers) {
          ...
          if (jsonData.offers['@type'] === 'AggregateOffer' && jsonData.offers.offers?.[0]) {
            price = parseFloat(jsonData.offers.offers[0].price) || parseFloat(jsonData.offers.lowPrice) || 0;
            currency = jsonData.offers.offers[0].priceCurrency || 'EUR';
            available = jsonData.offers.offers[0].availability === 'https://schema.org/InStock';
          } else {
            price = parseFloat(jsonData.offers.price) || 0;
            currency = jsonData.offers.priceCurrency || 'EUR';
            available = jsonData.offers.availability === 'https://schema.org/InStock';
          }

          if (price > 0) {
            console.log(`[${country}] Extracted from JSON-LD:`, { price, currency, available, name: metadata.name, imageUrl: metadata.imageUrl });
            return { price, currency, available, name: metadata.name, imageUrl: metadata.imageUrl };
          }
        }
      } catch (e) {
        // Continue to next JSON-LD block
    ...
    // Strategy 2: Look for window.jsan variable (IKEA's client-side data)
    const jsanMatch = html.match(/window\.jsan\s*=\s*({[\s\S]*?});?\s*<\/script>/);
    if (jsanMatch) {
      try {
        // Try to parse the jsan object
        const jsanText = jsanMatch[1];
        // Look for price in the jsan object
        const priceMatch = jsanText.match(/"price"\s*:\s*"?(\d+\.?\d*)"?/);
        if (priceMatch) {
          const price = parseFloat(priceMatch[1]);
          if (price > 0) {
            console.log(`[${country}] Extracted from jsan:`, price);
            return { price, currency: 'EUR', available: true, name: metadata.name, imageUrl: metadata.imageUrl };
          }
        }
      } catch (e) {
        console.error(`[${country}] Failed to parse jsan:`, e);
      }
    }

    // Strategy 3: Look for utag_data with improved regex
    const utagMatch = html.match(/var\s+utag_data\s*=\s*(\{[^;]+\});/);
    if (utagMatch) {
      try {
        const utagData = JSON.parse(utagMatch[1]);
        if (utagData.product_price || utagData.price) {
          const priceStr = utagData.product_price || utagData.price;
          const priceValue = Array.isArray(priceStr) ? priceStr[0] : priceStr;
          const price = parseFloat(String(priceValue).replace(',', '.'));
          if (price > 0) {
            console.log(`[${country}] Extracted from utag_data:`, price);
            return { price, currency: 'EUR', available: true, name: metadata.name, imageUrl: metadata.imageUrl };
          }
        }
      } catch (e) {
        console.error(`[${country}] Failed to parse utag_data:`, e);
      }
    }

    // Strategy 4: Direct regex search for price patterns in HTML
    const pricePatterns = [
      /"retailPrice"\s*:\s*"?(\d+\.?\d*)"?/,
      /"price"\s*:\s*"?(\d+\.?\d*)"?/,
      /"value"\s*:\s*(\d+\.?\d*)/,
    ...
    for (const pattern of pricePatterns) {
      ...
      if (priceMatch) {
        ...
        if (price > 0 && price < 100000) { // Sanity check
          console.log(`[${country}] Extracted from pattern ${pattern}:`, price);
          return { price, currency: 'EUR', available: true, name: metadata.name, imageUrl: metadata.imageUrl };
        }
      }
    }

    console.error(`[${country}] No price found with any strategy`);
    return null;
  } catch (error) {
    console.error(`[${country}] Error extracting price:`, error);
    return null;
  }
}

/**
 * Fetch product data for a specific country
 */
async function fetchProductData(country: keyof typeof COUNTRY_CONFIGS, productId: string): Promise<PriceData | null> {
  try {
    const url = buildProductUrl(country, productId);
    console.log(`[${country}] Fetching from:`, url);
    ...
    if (!response.ok) {
      console.error(`[${country}] HTTP ${response.status}`);
      return null;
    }

    const html = await response.text();
    const priceData = extractPriceFromHTML(html, country);

    if (!priceData) {
      console.error(`[${country}] Could not extract price`);
      return null;
    }

    console.log(`[${country}] Successfully extracted:`, priceData);
...
async function fetchStoreAvailability(
  ...
  try {
    ...
    console.log(`[${country}] Fetching availability for store ${buCode} (${storeName}) from:`, apiUrl);
    ...
    console.log(`[${country}] Got ${data.availabilities?.length || 0} store results`);
    ...
    if (!storeAvailability) {
      console.error(`[${country}] Store ${buCode} not found in availability results`);
      return null;
    }
    ...
    const cashCarryAvailability = storeAvailability.buyingOption?.cashCarry?.availability;
    let quantity = 0;
    let available = false;
    ...
    if (cashCarryAvailability) {
      ...
      const messageType = cashCarryAvailability.probability?.thisDay?.messageType;
      if (messageType) {
    ...
    console.log(`[${country}] Availability fetched for store ${buCode}:`, availability);
    return availability;
  } catch (error) {
 ...
 */
async function trackProductView(productId: string, prices: Record<string, PriceData>): Promise<void> {
  try {
 ...
 */
async function fetchAllPrices(productId: string): Promise<Record<string, PriceData>> {
  const countries: Array<keyof typeof COUNTRY_CONFIGS> = ['BE', 'NL', 'FR', 'DE'];

  console.log(`[Background] Fetching prices and availability for product ${productId} from all countries`);
  ...
  const availabilityPromises = countries.map(async (country) => {
    ...
    try {
      ...
      console.log(`[${country}] Fetching availability for store ${selectedStore.buCode} (${selectedStore.name})`);
      const availability = await fetchStoreAvailability(country, productId, selectedStore.buCode);
 ...
 * Message listener
 */
chrome.runtime.onMessage.addListener((message: FetchPricesMessage, sender, sendResponse) => {
  if (message.type === 'FETCH_PRICES') {
    console.log(`[Background] Received FETCH_PRICES request for product ${message.productId}`);
