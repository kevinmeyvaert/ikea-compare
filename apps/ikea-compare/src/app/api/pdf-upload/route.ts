...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[PDF Upload] Extracted text preview (first 1500 chars):', text.substring(0, 1500));
    ...
    console.log(`[PDF Upload] Found ${quantities.length} quantity markers and ${productCodes.length} product codes`);
    ...
    for (const productCode of productCodes) {
      ...
      for (let i = 0; i < quantities.length; i++) {
        if (quantities[i].index > productCode.index &&
      ...
      console.log(`[PDF Upload] Product ${productCode.code} -> quantity: ${quantity}`);
    ...
    console.log(`[PDF Upload] Found ${productIds.length} total products (${uniqueProductIds.length} unique):`, productIds);
