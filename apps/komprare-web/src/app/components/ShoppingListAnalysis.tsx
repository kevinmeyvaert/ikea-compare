...
const ShoppingListAnalysisComponent = memo(
  function ShoppingListAnalysisComponent({
    analysis,
    onReset,
  }: ShoppingListAnalysisProps) {
    ...
    const optimizedStrategy = useMemo(() => {
      ...
      analysis.products.forEach((product) => {
        ...
        if (cheapestStore && cheapestPrice !== Infinity) {
          const storeBreakdown = breakdown.find(
            (b) => b.store === cheapestStore
          );
          if (storeBreakdown) {
      ...
      const belgiumStore = analysis.singleStoreStrategy.all.find(
        (s) => s.storeCode === 'BE'
      );
    ...
    const exportToPDF = () => {
      ...
      doc.setTextColor(0, 0, 0);
      doc.text(
        `Totale kosten: €${optimizedStrategy.totalCost.toFixed(2)}`,
        14,
        63
      );
      doc.text(
        `Besparing t.o.v. enkel ${
          storeNames.BE
      ...
      optimizedStrategy.breakdown.forEach((store, storeIndex) => {
        ...
        currentY += 7;
        doc.text(
          `${store.productCount} producten • €${store.subtotal.toFixed(2)}`,
          14,
          currentY
        );
        currentY += 10;
      ...
      doc.setTextColor(0, 88, 163);
      doc.text(
        `TOTAAL: €${optimizedStrategy.totalCost.toFixed(2)}`,
        14,
        currentY
      );
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <div className="text-xs opacity-90 mb-1">
                Winkelen bij geselecteerde winkel
              </div>
            ...
            <div>
              <div className="text-xs opacity-90 mb-1">
                Je bespaart t.o.v. geselecteerde winkel
              </div>
              ...
              <div className="text-sm mt-1 opacity-90">
                {optimizedStrategy.belgiumTotal > 0
                  ? `${(
                      (optimizedStrategy.savings /
                        optimizedStrategy.belgiumTotal) *
                      100
                    ).toFixed(1)}% besparing`
                  : '0% besparing'}
              </div>
        ...
        <div className="bg-white border-2 border-gray-300 p-6">
          <h3 className="text-base font-semibold text-gray-900 mb-4">
            Selecteer winkels om te bezoeken
          </h3>
          <p className="text-sm text-gray-600 mb-4">
            Kies welke winkels je wilt bezoeken. We berekenen de beste
            winkelstrategie op basis van je selectie.
          </p>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-2xl">
                      {countryFlags[store.storeCode]}
                    </span>
                    <div>
                      <div className="font-bold text-gray-900">
                        {storeNames[store.storeCode]}
                      </div>
                      <div className="text-xs text-gray-500">
                        {store.storeName}
                      </div>
                      <div className="text-xs text-gray-600 mt-1">
                        {store.availableProducts} van {analysis.totalProducts}{' '}
                        items beschikbaar
                      </div>
          ...
          <div className="bg-blue-50 border-2 border-ikea-blue p-4">
            <h4 className="text-sm font-bold text-gray-900 mb-2">
              Hoe we beschikbaarheid hanteren
            </h4>
            <ul className="text-xs text-gray-700 space-y-1">
              <li className="flex items-start gap-2">
                <span className="text-ikea-blue font-bold">•</span>
                <span>
                  We nemen alleen items met prijzen mee in onze berekeningen
                  (items zonder prijzen worden als niet beschikbaar beschouwd)
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-ikea-blue font-bold">•</span>
                <span>
                  Als een item niet beschikbaar is bij je geselecteerde winkels,
                  wordt het niet meegenomen in de geoptimaliseerde kosten
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-ikea-blue font-bold">•</span>
                <span>
                  De optimalisatie kiest altijd de goedkoopste beschikbare optie
                  bij je geselecteerde winkels
                </span>
              </li>
            </ul>
          </div>
        ...
        <div className="bg-white border-2 border-gray-300 overflow-hidden">
          <div className="p-4 bg-gray-50 border-b-2 border-gray-300">
            <h3 className="text-base font-semibold text-gray-900">
              Jouw geoptimaliseerde winkelstrategie
            </h3>
            ...
            {optimizedStrategy.breakdown.map((store) => {
              ...
              const storeProducts = store.products
                .map((productId) => {
                  ...
                  const currentPrice =
                    product.products[storeMap[store.store]]?.price;
                  const productData = product.products[storeMap[store.store]];
              ...
              const cheapestHere = storeProducts.filter(
                (p) => p?.isCheapestOverall
              ).length;
              const notCheapestButAvailable =
                storeProducts.length - cheapestHere;
                      ...
                      <div className="flex items-center gap-3 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-gray-600">
                            {store.productCount} producten • €
                            {store.subtotal.toFixed(2)}
                          </div>
                          {notCheapestButAvailable > 0 && (
                            <div className="text-xs text-orange-600 mt-1">
                              {notCheapestButAvailable} item
                              {notCheapestButAvailable !== 1 ? 's' : ''} gekozen
                              omdat niet beschikbaar elders
                            </div>
                          )}
                        </div>
                      </div>
                              ...
                              <div
                                ...
                                className={`p-3 border-2 ${
                                  item.isCheapestOverall
                                    ? 'bg-ikea-pink-light border-ikea-pink'
                              ...
                              >
                                <div className="flex items-start justify-between gap-3">
                                  <div className="flex-1 min-w-0">
                                    ...
                                    {!item.isCheapestOverall && (
                                      <div className="text-xs text-orange-600 mt-1">
                                        ⚠️ Niet de goedkoopste, maar hier
                                        beschikbaar
                                      </div>
                                    )}
                                  </div>
                                  <div className="flex flex-col items-end flex-shrink-0">
                                    ...
                                    {item.isCheapestOverall && (
                                      <div className="text-xs text-ikea-pink font-medium mt-1">
                                        ✓ Beste prijs
                                      </div>
                                    )}
                                  </div>
        ...
        <div className="bg-white border-2 border-gray-300 overflow-hidden">
          <button
            ...
            className={`w-full p-4 bg-gray-50 hover:bg-gray-100 transition-colors text-left ${
              showFullTable ? 'border-b-2 border-gray-300' : ''
            }`}
          >
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-base font-semibold text-gray-900">
                  Volledige prijsvergelijking ({analysis.products.length}{' '}
                  producten)
                </h3>
                <p className="text-xs text-gray-600 mt-1">
                  {showFullTable
                    ? 'Klik om te verbergen'
                    : 'Klik om alle prijzen per land te zien'}
                </p>
              </div>
              <svg
                className={`w-5 h-5 text-gray-400 transition-transform ${
                        ...
                        >
                          ...
                          <td
                            className={`px-4 py-3 text-center text-sm ${
                              product.cheapest === 'BE'
                                ? 'bg-ikea-pink-light font-semibold text-ikea-pink'
                                : ''
                            }`}
                          >
                            {product.products.belgium?.price
                          ...
                          >
                            {product.products.netherlands?.price
                              ? `€${product.products.netherlands.price.toFixed(
                                  2
                                )}`
                              : '-'}
                          </td>
                          ...
                          >
                            {product.products.germany?.price
                              ? `€${product.products.germany.price.toFixed(2)}`
                              : '-'}
                          </td>
