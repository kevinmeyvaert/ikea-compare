...
import { logAnalyticsEvent, initializeAnonymousAuth, addToHistory } from '@ikea-compare/firebase';
import {
  ...
  const handleSearch = useCallback(
    async (searchProductId: string) => {
      ...
      try {
        ...
        const fetchAvailability = async (
          ...
          try {
            const availResponse = await fetch(
              `/api/availability/${searchProductId}?country=${country.toLowerCase()}&storeId=${
                store.buCode
              }`
            );
            ...
            if (availResponse.ok) {
              ...
              if (priceData.products[productKey]) {
                priceData.products[productKey]!.storeAvailability =
                  availability;
              }
            }
          } catch (err) {
        ...
        logAnalyticsEvent('product_compared', {
          product_id: searchProductId,
          product_name:
            priceData.products.belgium?.name ||
            priceData.products.netherlands?.name ||
            priceData.products.france?.name ||
            'Unknown',
        });
        ...
        // Add to history
        const anyProduct =
          priceData.products.belgium ||
          priceData.products.netherlands ||
          priceData.products.france;
        if (anyProduct) {
          const cheapestCountry = priceData.cheapest?.[0] as
            | 'BE'
            | 'NL'
            | 'FR'
            | undefined;
      ...
      <div className="flex flex-col lg:flex-row min-h-screen">
        ...
        <main className="flex-1 p-6 lg:p-8">
          <div className="max-w-6xl mx-auto">
            ...
            {/* Results */}
            {result && !loading && <ComparisonTable result={result} />}
